...
# TODO:  reconcile djOakData code with c4a
MinYear = 2014
MaxYear = 2018
...
OaklandBBox = [OakMinLng, OakMinLat, OakMaxLng, OakMaxLat]
...
def plotResults(request, beat, crimeCat, crimeCat2=None):
    ...
    # TODO precompute, cache city-wide stats for all crimeCat
    ...
    #		# TODO:  how to get figure from file?
    ...
    p.text(0.7, 0.95, annote, horizontalalignment='left', verticalalignment='bottom', transform=ax.transAxes,
           fontsize=6)
    ...
    logger.info(
        'user=%s plotting %d/%d (%6.2f sec) to %s' % (userName, totBeat, totCity, qryTime.total_seconds(), fullPath))

    # TODO: 181218  fix plot file permission
    # f1.savefig(fullPath,dpi=200)
...
def nearHere(request):
    ...
    if request.method == 'POST':
        ...
        logger.info('user=%s NearHere: NIncid=%d near (lat=%s,lng=%s)' % (
            userName, len(incidList), qryData['lat'], qryData['lng']))
...
def heatmap(request, mapType='general'):
    ...
    # TODO: mapbox unifies heatmap with circles
...
def hybridQual(request, mapType):
    ...
    # TODO: these queryset filters don't work?
...
class BeatAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        logger.info(
            'user=%s BeatListAPI %s nresult=%d (%6.2f sec) ' % (userName, beat, nresult, elapTime.total_seconds()))
...
class NearHereAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        elapTime = awareDT(datetime.now()) - begTime
        userName = self.request.user.get_username()
        logger.info('user=%s NearHereAPI lng=%s lat=%s nresult=%d (%6.2f sec)' % (
            userName, lngStr, latStr, nresult, elapTime.total_seconds()))
...
class CrimeCatAPI(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        elapTime = awareDT(datetime.now()) - begTime
        userName = self.request.user.get_username()
        logger.info('user=%s CrimeCatAPI cc=%s nresult=%d (%6.2f sec)' % (
            userName, crimeCat, nresult, elapTime.total_seconds()))
